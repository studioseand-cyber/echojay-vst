...
| kRegMaxSlots | LinkShm.h | 16 | 256 | 6 Sep 2026 (capacity ruling) | requirement 45 Links with headroom; 16 filled in one afternoon (16/16 live Pro Tools rows); region 65,600 B; the audio path walks a LIVE list, measured flat at 56 us/block for 0/16/50 rows |
| kDialSettleAttempts | PluginEditor.cpp (finishChain/EditBubbleWhenDialSettled) | 8 x 250 ms = 2,000 ms (literal 8 at two call sites) | 24 x 250 ms = 6,000 ms | 7 Sep 2026 (dialability C1') | DERIVED from the measurement: the longest map fetch in Sean's Pro Tools build turn was a 2,982 ms fallback lookup (429 ms and 481 ms were the other two samples; results_2026-09-06/protools_dial_log_20-57_build.log), so a real case ran out the 2 s bound and was worded as hand-dial. 6,000 ms = 2x the longest measured fetch, and it coincides with the existing 6 s EJDialSummary watchdog. Re-derive when a slower fetch is measured. |
| kRegLayoutVersion / registry file | LinkShm.h | (v1 in registry_v2.bin, unchecked) | v2 in registry_v3.bin, CHECKED on open (mismatch refused, EPROTO, logged) | 6 Sep 2026 | a foreign layout is never read through the wrong struct; old builds keep v2 |
| the walk / scan costs | measured | - | walk 4.3 ms fixed + 5 us/row; Link 30 Hz scan 1.1% -> 1.4% (50 rows) -> 2.3% (200) of one core | 6 Sep 2026 | said before it ships: 45 Links x 45 rows ~ 63% of one core in Link timers |
