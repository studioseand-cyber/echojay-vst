#!/bin/bash
# Build + run the DashboardWebFlow self-test against the real object code.
...
#
# It links the lib rather than recompiling DashboardWeb.cpp so the test exercises
# the shipping DashboardWebFlow::advance/currentUrl, not a re-implementation of
# them. DashboardWebFlow is pure (no webview), so no live page is involved.
set -e
cd "$(dirname "$0")/../.."
SCRATCH=$(mktemp -d)
...
cmd = (['clang++'] + out + ['-I', os.path.abspath('Source'),
        'tools/dashweb_test/dashweb_test.cpp',
        ...
        '-o', f'{scratch}/dashweb_test'])
r = subprocess.run(cmd, capture_output=True, text=True)
if r.returncode:
    print(r.stderr[-3000:])
    sys.exit(1)
PYEOF
"$SCRATCH/dashweb_test"
