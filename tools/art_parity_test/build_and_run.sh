...
#
# ProjectArt's members are static and free of editor state, so the linker can
# hand them over from the lib without an instance ever existing.
#
# The fixture path is passed explicitly, because the test resolves a relative
...
cmd = (['clang++'] + out + ['-I', os.path.abspath('Source'),
        'tools/art_parity_test/art_parity_test.cpp',
        ...
        '-o', f'{scratch}/art_parity_test'])
r = subprocess.run(cmd, capture_output=True, text=True)
if r.returncode:
    print(r.stderr[-3000:])
    sys.exit(1)
PYEOF
"$SCRATCH/art_parity_test" tools/art_parity_test/art-parity.json
