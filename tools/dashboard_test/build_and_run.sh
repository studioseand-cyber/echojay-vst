#!/bin/bash
# Build + run the Dashboard geometry self-test against the real objects.
# Usage: ./build_and_run.sh   (from anywhere; requires build/ to exist)
#
# Same shape as tools/tabstrip_test/build_and_run.sh: lift the real compile
# flags out of build/compile_commands.json so the test TU sees exactly the JUCE
# module defines and include paths the plugin is built with, then link the
# Release SharedCode static lib for the symbols.
#
# It links the lib rather than recompiling DashboardTab.cpp so the test
# exercises the object code that ships. A separately compiled copy would be
# the harness-verifies-a-reimplementation trap the art parity fixture fell into.
...
cmd = (['clang++'] + out + ['-I', os.path.abspath('Source'),
        'tools/dashboard_test/dashboard_test.cpp',
        ...
        '-o', f'{scratch}/dashboard_test'])
r = subprocess.run(cmd, capture_output=True, text=True)
if r.returncode:
    print(r.stderr[-3000:])
    sys.exit(1)
PYEOF
"$SCRATCH/dashboard_test"
