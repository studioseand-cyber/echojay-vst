#!/bin/bash
# Build + run structplan_test: the rack lock's pure decisions (RackLock::read /
...
cmd = (['clang++'] + out + ['-I', os.path.abspath('Source'),
        'tools/structplan_test/structplan_test.cpp',
        ...
        '-o', f'{scratch}/structplan_test'])
r = subprocess.run(cmd, capture_output=True, text=True)
if r.returncode:
    print(r.stderr[-4000:])
    sys.exit(1)
PYEOF
"$SCRATCH/structplan_test" "$SCRATCH"
