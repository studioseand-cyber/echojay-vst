#!/bin/bash
# Build + run the typedReadbackMatch self-test against the real header.
# Usage: ./build_and_run.sh   (from anywhere; requires build/ to exist)
#
# Same shape as tools/dashboard_test on the dashboard branch: lift the real
# compile flags out of build/compile_commands.json so the test TU sees exactly
# the JUCE module defines and include paths the plugin is built with, then
# link the Release SharedCode static lib for the JUCE symbols.
#
...
cmd = (['clang++'] + out + ['-I', os.path.abspath('Source'),
        'tools/bridged_readback_test/bridged_readback_test.cpp',
        ...
        '-o', f'{scratch}/bridged_readback_test'])
r = subprocess.run(cmd, capture_output=True, text=True)
if r.returncode:
    print(r.stderr[-3000:])
    sys.exit(1)
PYEOF
"$SCRATCH/bridged_readback_test"
