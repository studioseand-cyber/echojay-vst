...
public:
    ...
    void prepare (double sampleRate, int /*maxBlockSize*/)
    {
        ...
        {
            // Full length plus modulation headroom; `size` only ever shortens.
        ...
        for (int i = 0; i < kNumDiff; ++i)
        {
            ...
            diffL_[i].setGain (kDiffGain[i]);
            diffR_[i].setGain (kDiffGain[i]);
        }
    ...
    {
        return 3.0 + 20.0 * (double) sizePct_.load (std::memory_order_relaxed) * 0.01;
    }
    ...
    void process (float* left, float* right, int numSamples) noexcept
    {
        ...
        for (int n = 0; n < numSamples; ++n)
        {
            ...
            // Feedback: per-line decay gain, damping, low cut, then the matrix.
            float f[kNumLines];
            for (int i = 0; i < kNumLines; ++i)
                f[i] = lineGain_[i] * hp_[i].process (lp_[i].process (y[i]));
...
private:
    ...
    void recompute() noexcept
    {
        ...
        const double dampHz  = dampingCutoffHz ((double) dampingPct_.load (std::memory_order_relaxed));
        const double lowCut  = (double) lowCutHz_.load (std::memory_order_relaxed);

        for (int i = 0; i < kNumLines; ++i)
        {
            const double seconds = kLineMs[i] * 0.001 * sizeF;

            targetLineSamples_[i] = std::max (8.0, seconds * sampleRate_);
            lineGain_[i]          = decayGain (seconds, rt60);
        ...
        for (int k = 0; k < kNumTaps; ++k)
        {
            tapSamplesL_[k] = kTapMsL[k] * 0.001 * sizeF * sampleRate_;
            tapSamplesR_[k] = kTapMsR[k] * 0.001 * sizeF * sampleRate_;
        }
    }
    ...
    // Diffusion allpasses (Dattorro-style: two short, two longer).
    ...
    Smoother         predelaySmooth_, mixSmooth_, widthSmooth_, elSmooth_;
