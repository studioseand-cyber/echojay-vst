...
// sub-tab DRAWS IT since 20 Sep 2026 (EJMatchPage.h has that screen's geometry
// and its sentences), so computeMatchProposal now has two callers: the page and
// tools/mapfps_test.
//
// matchPageStatement WENT WITH THAT COMMIT. It was the page's promise of what
// Match would show, and its last line said this version computes no proposal.
// Both stopped being true the moment the page drew one, and a statement nothing
// draws is a dead function, so it was removed rather than left to rot beside
// the thing that replaced it. mr PIN12 moved with it, onto the real sentences.
//
