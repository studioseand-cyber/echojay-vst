        ...
        if (statusCode == 200 && json.isObject())
        {
            ...
            if (obj && obj->hasProperty("token"))
            {
                ...
                if (obj->hasProperty("plan"))
                    userInfo.isPro = obj->getProperty("plan").toString() == "pro";
                ...
                if (userInfo.messageLimit <= 0)
                    userInfo.messageLimit = userInfo.isPro ? 100 : 3;
                           ...
                           std::function<void(const juce::String& reply, bool success)> onComplete)
{
    if (!canSendMessage())
    {
        juce::String limit = userInfo.isPro ? "100" : "3";
        onComplete("You\u2019ve hit your daily limit of " + limit + " AI messages. " +
                   (userInfo.isPro ? "Limit resets at midnight." : "Upgrade to Pro for 100 messages per day."), false);
        return;
    }
    ...
    postJSON("/api/chat", body, [this, onComplete](const juce::var& json, int statusCode)
    {
        ...
        if (statusCode == 429)
        {
            onComplete(juce::String("Daily message limit reached. ") + 
                       (userInfo.isPro ? "Resets at midnight." : "Upgrade to Pro for 100/day."), false);
            return;
        }
...
void EchoJayAPI::loadSettings()
{
    ...
    if (auto* obj = json.getDynamicObject())
    {
        ...
        authToken = obj->getProperty("token").toString();
        userInfo.email = obj->getProperty("email").toString();
        userInfo.isPro = (bool)obj->getProperty("isPro");
        userInfo.messageLimit = userInfo.isPro ? 100 : 3;
...
void EchoJayAPI::saveSettings() const
{
    ...
    obj->setProperty("isPro", userInfo.isPro);
